...
export default (client: Client, dbclient: MongoClient): void => {
    ...
    client.on('guildMemberUpdate', async (oldMember, newMember): Promise<void> => {
        ...
        const hadRole = oldMember.roles.resolve((await guild.roles.fetch()).find(role => role.name.toLowerCase().includes("Nitro Booster")));
        const hasRole = newMember.roles.resolve((await guild.roles.fetch()).find(role => role.name.toLowerCase().includes("Nitro Booster")));
        if (!hadRole && hasRole) {
